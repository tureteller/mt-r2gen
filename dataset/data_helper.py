...
class FieldParser:
    ...
    def parse(self, features):
        to_return = {'id': features['historical_report']['id']}  # 使用 historical_report 的 id
        historical_report = features['historical_report']['report']
        current_report = features['current_report']['report']
        
        # 清洗历史和当前报告
        historical_report = self.clean_report(historical_report)
        current_report = self.clean_report(current_report)
        
        to_return['historical_input_text'] = historical_report
        to_return['current_input_text'] = current_report
        
        # 加载图像
        images = []
        for image_path in features['historical_report']['image_path']:
            with Image.open(os.path.join(self.args.base_dir, image_path)) as pil:
                ...
                images.append(image)
        for image_path in features['current_report']['image_path']:
            with Image.open(os.path.join(self.args.base_dir, image_path)) as pil:
                array = np.array(pil, dtype=np.uint8)
                if array.shape[-1] != 3 or len(array.shape) != 3:
                    array = np.array(pil.convert("RGB"), dtype=np.uint8)
